...
import { Trophy, CircleCheck, Calendar, Quote, Copy, Check } from 'lucide-react';
...
export const ShareCard: React.FC<ShareCardProps> = ({ progress, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="relative w-full max-w-sm rounded-[32px] overflow-hidden shadow-2xl group bg-zinc-900 border border-zinc-800">
        
        {/* Close Button */}
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 z-30 p-2 bg-black/20 text-white/80 rounded-full hover:bg-black/50 transition-colors backdrop-blur-md"
        >
          ✕
        </button>

        {/* Card Content */}
        <div className="relative aspect-[9/16] flex flex-col justify-between overflow-hidden">
