...
import { 
  ...
  Share2, Save, Bell, BellOff, Clock, Check, 
  CircleAlert, Cloud, CloudOff, LogOut, Loader2, Mail, LockKeyhole,
  Sparkles, X, Eye, Lock
} from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const [showAuthUI, setShowAuthUI] = useState(false); // Control the Auth Modal
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'signup') {
        ...
        setShowAuthUI(false); // Close modal on success
      }
    } catch (err: any) {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Removed window.location.reload() to prevent 404s.
      // The App.tsx listener will handle the state update naturally.
    }
  };
      ...
      <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="grid gap-3">
          {[
            ...
            { 
              id: 'account', 
              icon: userSession ? Cloud : User, // Show Cloud if synced, User if guest
          ...
          <div className="rounded-xl border border-zinc-800 overflow-hidden">
             {userSession ? (
               ...
               <div className="p-5 bg-zinc-900/50">
                  <div className="flex items-center gap-4 mb-5">
                     <div className="p-3 bg-zinc-800 text-zinc-400 rounded-full shrink-0">
                        <CloudOff size={20} />
                     </div>
                     <div>
                        <h3 className="font-bold text-zinc-300 text-sm">Guest Mode</h3>
                        <p className="text-xs text-zinc-500">Data stored locally on this device.</p>
                     </div>
                  </div>
                  <button 
                    onClick={() => setShowAuthUI(true)}
                    className="w-full py-3 bg-white hover:bg-zinc-200 text-black font-bold text-xs rounded-xl transition-colors shadow-lg shadow-white/10"
                  >
                    Sign In / Sync Data
                  </button>
               </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
               <div className="relative w-full max-w-sm bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                  ...
                  <button 
                    ...
                    className="absolute top-4 right-4 z-30 p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-full transition-colors"
                  >
                    <X size={20} />
                  </button>

                  <div className="p-8 text-center">
                     <div className="w-12 h-12 bg-zinc-900 rounded-xl mx-auto flex items-center justify-center border border-zinc-800 mb-4">
                        <Cloud className="text-emerald-500 w-6 h-6" />
                     </div>
                     
                     <h2 className="text-xl font-bold text-white mb-2">
                        {authMode === 'signin' ? 'Cloud Sync' : 'Create Account'}
                     </h2>
                     <p className="text-xs text-zinc-500 mb-6 px-4">
                        {authMode === 'signin' ? 'Sign in to access your data.' : 'Secure your progress in the cloud.'}
                     </p>
                     ...
                     <form onSubmit={handleAuth} className="space-y-3 text-left">
                        <div className="relative group">
                           ...
                           <input
                             ...
                             className="w-full bg-zinc-900/50 border border-zinc-800 rounded-xl pl-9 pr-4 py-2.5 text-white focus:border-emerald-500 focus:outline-none text-sm placeholder:text-zinc-600"
                           />
                        </div>
                        <div className="relative group">
                           <LockKeyhole className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500" size={14} />
                           <input
                             type="password"
                             required
                        ...
                        {authError && (
                           <div className="p-2.5 bg-red-950/20 border border-red-900/30 rounded-lg flex items-center gap-2 text-red-400 text-[10px] animate-in fade-in">
                        ...
                        <button
                          ...
                          className="w-full py-2.5 bg-white hover:bg-zinc-200 text-black font-bold rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50 text-sm mt-2"
                        >
                          {authLoading ? <Loader2 className="animate-spin" size={16} /> : (authMode === 'signin' ? 'Sign In' : 'Create Account')}
                        </button>
                     </form>
